...
export async function triggerDailyMidnightReconciliation(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(200).json({
      message: 'Daily 12:45 AM IST reconciliation sync completed',
      scheduledTimeIST: '12:45 AM IST (19:15 UTC)',
      data: result,
    });
  } catch (error: any) {
    res.status(500).json({ error: 'Failed to execute daily 12:45 AM IST reconciliation' });
  }
}
...
export async function getSyncStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(200).json({
      ...
      dailyReconciliationIST: '12:45 AM IST (19:15 UTC)',
