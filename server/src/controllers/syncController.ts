...
export async function triggerDailyMidnightReconciliation(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(200).json({
      message: 'Daily 2:15 AM IST reconciliation sync completed',
      scheduledTimeIST: '2:15 AM IST (20:45 UTC)',
      data: result,
    });
  } catch (error: any) {
    res.status(500).json({ error: 'Failed to execute daily 2:15 AM IST reconciliation' });
  }
}
...
export async function getSyncStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(200).json({
      ...
      dailyReconciliationIST: '2:15 AM IST (20:45 UTC)',
